...
/* The router contains the calls to all the functions based on the value of the first item in the array */
char *router(char *array[], int line)
{
	...
	if the value of array[0] does not equal one of the values listed above, then it must be a label
	the program then calls the labelreader, to reference the symbol table on what to do next.
	 */
	else
	{
